...
{
    public Task<string> Handle(SlowRequest request, CancellationToken cancellationToken)
...
{
    public Task<string?> Handle(NullableRequest request, CancellationToken cancellationToken)
...
public class MediatorEdgeCaseTests
{
    ...
    public void Send_WithAlreadyCancelledToken_HandlerReceivesCancelledToken()
    {
        ...
        var provider = new FakeServiceProvider()
            .Register<IRequestHandler<PingRequest, PongResponse>>(handler);
        var mediator = new MediatorClass(provider);

        var task = mediator.Send(new PingRequest("test"), cts.Token);
        Assert.ShouldNotThrowAsync(() => task);
    }
