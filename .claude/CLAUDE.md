...
4. `NotificationHandlerWrapper<TNotification>` follows the same lazy-cache pattern for `Publish`, but fans out to *all* registered `INotificationHandler<TNotification>` sequentially, collecting non-cancellation exceptions and throwing them together as an `AggregateException` once the loop finishes. If a handler throws `OperationCanceledException`/`TaskCanceledException`, the loop stops immediately (later handlers don't run) and that exception propagates — bare, if no earlier handler in the same `Publish` call had already failed, or folded into the `AggregateException` alongside the earlier failure(s) if one had, so a real handler failure is never silently dropped just because a later handler was cancelled.
