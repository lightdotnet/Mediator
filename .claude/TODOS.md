...
2. **[breaking — architectural, needs explicit decision] Handler/behavior interfaces (`IRequestHandler`, `ICommandHandler`, `IQueryHandler`, `INotificationHandler`, `IPipelineBehavior`) live in `src/Mediator`, not `src/Mediator.Contracts`.**
...
3. **[breaking — flag only, not recommended] `class Mediator` shares its name with the `Light.Mediator` namespace, forcing consumers (including this repo's own tests) to alias it.**
